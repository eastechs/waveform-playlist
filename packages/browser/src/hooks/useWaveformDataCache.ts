 ...
 *
 * Manages a Map<clipId, WaveformDataObject> that stores pre-computed peaks
 * objects generated by a web worker. These enable near-instant zoom changes
 * via resample() instead of re-scanning raw audio samples.
 *
 * Behavior:
...
export interface UseWaveformDataCacheReturn {
  cache: ReadonlyMap<string, WaveformDataObject>;
...
export function useWaveformDataCache(
  ...
  const [cache, setCache] = useState<Map<string, WaveformDataObject>>(() => new Map());
  const [isGenerating, setIsGenerating] = useState(false);

  const workerRef = useRef<PeaksWorkerApi | null>(null);
  const generatedByBufferRef = useRef<WeakMap<AudioBuffer, WaveformDataObject>>(new WeakMap());
  ...
  useEffect(() => {
    ...
    const immediateAssignments: Array<{ clipId: string; waveformData: WaveformDataObject }> = [];
